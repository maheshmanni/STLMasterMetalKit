...
struct Uniforms {
    ...
    init(modelMatrix: float4x4, viewMatrix: float4x4, projectionMatrix: float4x4,
         cameraPosition: float3, lightDirection: float3, lightPosition: float3, diffuseColor : float4)
    {
...
class Node {
    ...
    init(mesh: MTKMesh, materials: [Material]) {
        ...
        diffuseColor = float4(1.0, 0.85, 0.57, 1.0)
    }
}
...
class Renderer: NSObject, MTKViewDelegate {
    ...
    var baseColor = float4(1.0, 0.85, 0.57, 1.0)
    var time: Float = 0
    ...
    init(view: MTKView, device: MTLDevice) {
        ...
        irradianceCubeMap = Renderer.buildEnvironmentTexture("lightroom_14b_ibl.ktx", device: device)
        super.init()
        
//        guard let modelURL = Bundle.main.url(forResource: "ring", withExtension: "stl") else {
...
//        buildScene(url: modelURL, device: device, vertexDescriptor: vertexDescriptor)
    ...
    func LoadModelAtUrl(url : URL) {
        
        buildScene(url: url, device: self.device, vertexDescriptor: vertexDescriptor)
    }
...
//                                       tangentAttributeNamed: MDLVertexAttributeTangent)
    ...
    func setBaeColor(color : float4) {
        //baseColor = color
        if(nodes.count > 0)
        {
            nodes[nodes.count - 1].diffuseColor = color
        }
    }
    ...
    func updateScene(view: MTKView) {
        ...
        var direction  = viewMatrix.columns.2
        
        //cameraWorldPosition = float3(0.0, 0.0, 10.0)
       // float3 dir(direction.xyz);
     //   cameraWorldPosition = cameraWorldPosition + (direction.xyz * 10)
        lightWorldPosition = cameraWorldPosition
        lightWorldDirection = direction.xyz//normalize(cameraWorldPosition)
    }
    ...
    func getUIImage(texture: MTLTexture, context: CIContext) -> UIImage?{
        ...
        if let ciImageFromTexture = CIImage(mtlTexture: texture, options: nil) {
    ...
    func draw(in view: MTKView) {
      ...
      //      self.currentTexture = view.currentDrawable!.texture
