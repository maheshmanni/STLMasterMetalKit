                             ...
                             texturecube<float> irradianceMap [[texture(textureIndexIrradiance)]])
{
    ...
    float4 baseColor = uniforms.baseColor;//float4(1.0, 0.85, 0.57, 1.0);//baseColorMap.sample(linearSampler, in.texCoords);
    parameters.baseColor = linear_from_srgb(baseColor.rgb);
    parameters.roughness = 0.3;//roughnessMap.sample(linearSampler, in.texCoords).g;
    parameters.metalness = 1.0;//metallicMap.sample(linearSampler, in.texCoords).b;
    ...
    float3 color = diffuseTerm(parameters) + specularTerm(parameters);
